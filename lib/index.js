...
export function apply(ctx, config = {}) {
  ...
  const sync = (enabled) => {
    if (enabled && !skillEffect) {
      try {
        ...
        registered = true;
        log(`consent 已开启，技能 "${SKILL_NAME}" 已注册。`);
      } catch (error) {
        log(`技能注册失败：${error}`);
      }
  ...
  ctx.inject(["settings"], (sctx) => {
    const scope = sctx.settings.register(NS, schema, {
      base: { consent: config?.consent === true }
    });
    sync(scope.get()?.enabled ?? false);
    scope.watch(() => sync(scope.get()?.enabled ?? false));
  });
