...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var DICT_ZH = {
      ...
      description: "注册 dsh-plugin-publishing 工作流技能（DSH 插件开发 → GitHub 发布）。技能默认启用（保存后刷新保持启用），仅在设置页主动取消勾选后才停用。",
      enabledLabel: "启用技能",
      enabledHint: "默认开启。开启后本会话的技能目录中会出现 dsh-plugin-publishing；取消勾选即停用，保存后刷新仍保持你的选择。",
      enabledOn: "已启用",
      enabledOff: "未启用",
      ...
      loading: "正在加载设置…（模式: {mode}）",
      unavailable: "设置不可用（模式: {mode}）——请确认以 127.0.0.1/localhost 访问本页",
    ...
    var DICT_EN = {
      ...
      description: "Registers the dsh-plugin-publishing workflow skill (DSH plugin development → GitHub publishing). Enabled by default (stays enabled across refreshes once saved); only an explicit uncheck in this page disables it.",
      enabledLabel: "Enable skill",
      enabledHint: "On by default. When on, the dsh-plugin-publishing skill appears in this session's skill catalog; unchecking disables it, and your choice persists across refreshes.",
      enabledOn: "Enabled",
      enabledOff: "Disabled",
      tokenLabel: "GitHub PAT (fine-grained Personal Access Token)",
      ...
      loading: "Loading settings… (mode: {mode})",
      unavailable: "Settings unavailable (mode: {mode}) — make sure you access this page via 127.0.0.1/localhost",
    ...
    // ---- Controller: plain-HTTP bridge to the host routes ----
    // The card talks to /api/dsh-plugin-publisher/* directly instead of the
    // client settings/credentials RPC channels, which keeps it working even
    // when those channels are unavailable in a deployment.
    function makeController() {
      var API = "/api/dsh-plugin-publisher";
      var HEADERS = { "x-dsh-plugin-publisher": "1" };
      var stagedEnabled = null; // null = no change
      var stagedToken = "";    // "" = no change
      var listeners = [];
      var retryTimer = null;
      var retries = 0;
      // Non-null initial snapshot so the first render is always valid.
      var state = {
        ready: false,
        writable: true,
        status: "loading",
      ...
      function publish() {
        state = {
          ...
          status: state.status,
          mode: "http",
          enabled: stagedEnabled === null ? state.enabled : stagedEnabled,
          dirty: stagedEnabled !== null || stagedToken !== "",
          saving: state.saving,
          failed: state.failed,
          message: state.message,
          tokenConfigured: state.tokenConfigured,
          tokenWritable: state.tokenWritable,
          tokenDraft: stagedToken,
      ...
      async function load() {
        try {
          ...
          if (json.ok) {
            state.ready = true;
            state.status = "ready";
            state.enabled = Boolean(json.resolved && json.resolved.enabled);
            state.tokenConfigured = Boolean(json.tokenConfigured);
            state.rawValue = json.resolved ? JSON.stringify(json.resolved).slice(0, 120) : "";
          } else {
            state.status = "error";
          }
        } catch (error) {
      ...
      // Bounded retry until the host responds.
      function ensureLoaded() {
        if (state.ready) return;
        if (retries >= 30) return;
        retries += 1;
        retryTimer = setTimeout(function () {
          void load().then(ensureLoaded);
        }, 600);
      }
      ...
      return {
        ...
        save: async function () {
          ...
          publish();
          try {
            if (stagedEnabled !== null) {
              var enabledResponse = await fetch(API + "/enabled", {
                method: "POST",
                headers: HEADERS,
                body: JSON.stringify({ enabled: stagedEnabled })
              });
              if (!enabledResponse.ok) throw new Error("enabled write failed (HTTP " + enabledResponse.status + ")");
            }
            if (stagedToken !== "") {
              ...
              if (!tokenResponse.ok) throw new Error("token write failed (HTTP " + tokenResponse.status + ")");
            }
            stagedEnabled = null;
            stagedToken = "";
            await load();
    ...
    function PublisherCard(props) {
      ...
      return h("div", { style: cardStyles.root },
        ...
        !state.ready ? h("p", { style: cardStyles.msg }, t(state.status === "unavailable" ? "unavailable" : "loading", { mode: state.mode }) + (state.rawValue ? " · raw=" + state.rawValue : "")) : null
      );
    }
    ...
    function apply(ctx) {
      ...
      // Dispose the retry timer with the plugin fiber.
      if (typeof ctx.effect === "function") {
        ctx.effect(() => () => controller.dispose(), "dsh-plugin-publisher: controller dispose");
      }
