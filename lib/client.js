...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function makeController(ctx) {
      ...
      // Decode override: read the host-resolved value directly instead of
      // round-tripping through the wire schema envelope, so the toggle always
      // reflects the persisted `enabled` state regardless of schema serialization.
      var scope = ctx.settingsScope.bind({
        namespace: NS,
      ...
      // Mutable side state; `state` itself is REPLACED on every publish so
      // useSyncExternalStore sees a new snapshot reference and re-renders.
      var meta = {
        saving: false,
        failed: false,
        message: "",
      ...
      function publish() {
        ...
        state = {
          ...
          mode: snapshot.mode,
          enabled: stagedEnabled === null ? Boolean(snapshot.value && snapshot.value.enabled) : stagedEnabled,
          dirty: stagedEnabled !== null || stagedToken !== "",
          saving: meta.saving,
          failed: meta.failed,
          message: meta.message,
          tokenConfigured: meta.tokenConfigured,
          tokenWritable: meta.tokenWritable,
          tokenDraft: stagedToken,
          rawValue: snapshot.value ? JSON.stringify(snapshot.value).slice(0, 120) : String(snapshot.status)
        };
        for (var i = 0; i < listeners.length; i++) listeners[i](state);
      }

      // Bounded retry: a boot-time RPC race can leave the scope stuck
      // "loading" (which would render the toggle unchecked forever). Re-queue
      // host reads until the namespace is ready or unavailable.
      ...
      function refreshCredential() {
        api.credentials.describe({ refs: [tokenRef] }).then(function (resp) {
          ...
          meta.tokenConfigured = Boolean(view && view.configured);
          meta.tokenWritable = view ? view.writable : true;
          publish();
        }).catch(function () {
          publish();
        });
